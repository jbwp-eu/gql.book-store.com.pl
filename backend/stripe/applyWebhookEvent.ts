...
import { onStripeOrderNewlyPaid } from "../utils/onOrderNewlyPaid.js";
...
export function applyStripeWebhookEvent(event: Stripe.Event): void {
  switch (event.type) {
    case "charge.succeeded": {
      ...
      if (paymentIntentId) {
        onStripeOrderNewlyPaid(setOrderPaidByStripePaymentIntentId(paymentIntentId));
      }
      break;
    }
    case "payment_intent.succeeded": {
      ...
      if (pi.id) {
        onStripeOrderNewlyPaid(setOrderPaidByStripePaymentIntentId(pi.id));
      }
      break;
    }
