...
ssh -i ~/gql-book-store-key.pem ubuntu@<PUBLIC_IP>
# or after DNS:
ssh -i ~/gql-book-store-key.pem ubuntu@gql.book-store.pl
```
...
Use [Caddyfile.example](Caddyfile.example) (hostname `gql.book-store.pl` → `127.0.0.1:4000`).
...
| Secret                                      | Value                                                                 |
| ------------------------------------------- | --------------------------------------------------------------------- |
| `EC2_HOST`                                  | `gql.book-store.pl` (or public IP until DNS works)                    |
...
| `VITE_STRIPE_PUBLISHABLE_KEY_TEST_MODE_AWS` | Stripe **publishable** key (`pk_…`) for checkout (AWS Stripe account) |
| `VITE_PAYPAL_CLIENT_ID`                     | PayPal client id for checkout                                         |


CI injects these at **frontend build** time (see `env:` on **Install and build** in `.github/workflows/deploy-ec2-v2.yml`). They are not read from `frontend/.env.local` on the runner. Workflow sets `VITE_DEPLOY_TARGET=aws`.
...
| `DEPLOY_BASE_URL_AWS`       | `https://gql.book-store.pl` (enables post-deploy smoke test in CI)                  |
...
```bash
curl -sS https://gql.book-store.pl/
curl -sS -X POST https://gql.book-store.pl/graphql \
...
```bash
DEPLOY_BASE_URL_AWS=https://gql.book-store.pl bash deploy-ver.2/smoke-test.sh
```
...
Stripe webhook URL: `https://gql.book-store.pl/webhooks/stripe`
